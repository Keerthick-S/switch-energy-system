...
import switch_energy_system.backend.dto.LoginRes;
...
public class LoginController {
    ...
    public LoginRes authenticateAndGetToken(@RequestBody AuthRequest authRequest) {
        Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(authRequest.getUserName(), authRequest.getPassword()));
        User user = userRepo.getUserById(authRequest.getUserName());
        if (authentication.isAuthenticated()){
            String token = jwtService.generateToken(authRequest.getUserName());
            return new LoginRes(token, user.getRole(), user.getId());
        }
        else {
